...
3. Rolling Dice Simulation - In the rolling dice simulation, the user first picks a number of die that they want to roll, needing to be within a range of 1 to 6. From there the program checks what number is inputted and then rolls the number of die that the user has picked. This is done by choosing a random number between 1 and 6 and then repeating this for the number of die chosen in the beginning. The results of the random numbers generated are then outputted to the user in a list.
