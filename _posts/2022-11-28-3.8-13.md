...
It is important to know that functions can look different and still do the same things because there are many different ways to code something but the final products is still the exact same. This shows that some programs can be complex while other are simpler, allowing coders to eventually achieve the most simple way of coding a function. Some functions can look the same while 
